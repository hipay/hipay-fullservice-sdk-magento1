...
class Allopass_Hipay_Model_Method_Hosted extends Allopass_Hipay_Model_Method_HostedAbstract
{
    ...
    {
        if ($payment->getOrder()->getBillingAddress()->getCountry() == 'BE') {
            $paymentProductList = $this->getConfigData('cctypes');
        } else {
            $paymentProductList = str_replace('bcmc', '', $this->getConfigData('cctypes'));
        }

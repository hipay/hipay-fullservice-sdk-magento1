...
if [ "$1" = '' ] || [ "$1" = '--help' ]; then
    ...
    #casperjs test $pathLibHipay $pathPreFile ${pathDir}/[0-1]*/202-*.js --url=$BASE_URL --url-mailcatcher=$URL_MAILCATCHER --login-backend=$LOGIN_BACKEND --pass-backend=$PASS_BACKEND --login-paypal=$LOGIN_PAYPAL --pass-paypal=$PASS_PAYPAL --xunit=${header}result.xml --ignore-ssl-errors=true --ssl-protocol=any
